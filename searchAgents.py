    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nxt = (nextx, nexty)
                cost = self.costFn(nxt)
                successors.append( ( nxt, action, cost) )
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append(((next_state, tuple(corner_state)), action, cost))
...
def cornersHeuristic(state, problem):
    ...
    undiscovered = []
    for idx in range(len(corners)): # iterate through all the corners and store those with 0. 
        if corner_state[idx] == 0:
            undiscovered.append(corners[idx])
    current = position
    cost = 0
    while len(undiscovered) != 0:
        idx, dist = findClosestCorner(current, undiscovered)
        cost += dist
        current = undiscovered[idx]
        undiscovered.remove(undiscovered[idx])
    return cost

def findClosestCorner(position, corners):
    smallest = None
    idx = -1
